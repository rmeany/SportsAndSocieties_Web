var config = {
  ...
  var tblUsers = document.getElementById('tbl_fixture_list');
...
function myFunction() {
  ...
  table = document.getElementById("tbl_fixture_list");
...
// get Sports Clubs
	...
	function database(categoryText, clubText) {
	  databaseRef.child(categoryText).child(clubText).child('fixtures').once('value', function(snapshot) {
	  	if(snapshot.exists()){
	  		var content = '';
	  		...
	  		snapshot.forEach(function(childSnapshot) {
	            ...
	            content += '<td class="opp">' + opposition + '</td>';//column3
	            content += '<td class="loc">' + location + '</td>';//column4
	            content += '<td class="time">' + time + '</td>';//column5
	            content += '<td class="type">' + type + '</td>';//column6
	            content += '<td><button id="updatebtn' + rowIndex + '" type="button" class="update_fixture" onclick="updateButton()">Update</button></td>';//update button
	            content += '<td><button id="removebtn' + rowIndex + '" type="button" class="remove_fixture" onclick="removeButton()">Remove</button></td>';//remove button
	            content += '</tr>';

	            rowIndex = rowIndex + 1
	        });

	  		$("#tbl_fixture_list tbody tr").remove(); 
	        $('#tbl_fixture_list').append(content);
	  	}
	  	else {
	  		// show alert
			document.querySelector('#fixtureAlert').style.display = 'block';

			// hide alert after 3 seconds
			setTimeout(function() {
			    document.querySelector('#fixtureAlert').style.display = 'none';
			},3000);
	...
	function updateButton() {
		...
		$(".update_fixture").click(function(e){
				...
				if (validation(team, opposition, type,location) == false){
				   ...
				   || location.includes(forSlash) || location.includes(dot) || location.includes(dollar) || location.includes(hash) || location.includes(openBrack) || location.includes(closeBrack))
				  {
				    ...
				    return false;
						...
						var updateRef = databaseRef.child(getCategoryText()).child(getClubText()).child('fixtures');
						updateRef.child(opposition + " " + date).once('value', function(snapshot) {
						  if (snapshot.exists()) {
						  	// confirm submission
							  var r = confirm("A fixture against '" + opposition + "' on '" + date + "' already exists. Do you want to overwite the existing information?");
							    if (r == true) {
							    ...
							    .then(function() {
							      $row.remove();
									...
									var date = $("#myModal").find('#modalDate').val();
									var time = $("#myModal").find('#modalTime').val();
							      var EventRef = databaseRef.child(getCategoryText()).child(getClubText()).child('fixtures').child(opposition + " " + date).
									set({
										team: team,
	...
	function removeButton() {
		...
		var rows = document.getElementById('tbl_fixture_list').rows.length;

			$(".remove_fixture").click(function(e){
		    var $row = $(this).closest("tr");
		    ...
		    if (rows > 2) {
		    	var r = confirm("Are you sure you want to remove " + $rowOpposition + " " + $rowDate + " ?");
					    if (r == true) {
							var newFixturesRef = databaseRef.child(getCategoryText()).child(getClubText()).child('fixture');
							newFixturesRef.child($rowOpposition + " " + $rowDate).remove()
						    .then(function() {
						      $row.remove();
						      reload_page();
						    })
